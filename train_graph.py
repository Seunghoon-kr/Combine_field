...
if __name__ == '__main__':    
    ...
    #data file 가져오기
    train_data = torchvision.datasets.ImageFolder(root='image/class/train_black_data',transform=trans)
    test_data = torchvision.datasets.ImageFolder(root='image/class/test_black_data',transform=trans)
    ...
    net = CNN().to(device)
    #network size test
    test_input = (torch.Tensor(4,3,224,224).to(device))
    ...
    for epoch in range(epochs):
        ...
        if avg_cost <= 0.01:
            torch.save(net.state_dict(), "./weight/black_img/class4/model_c4_({})_({})_({}).pth".format(learning_rate, epoch, round(val_accuracy.item()*100,2)))
        elif val_accuracy >= 0.94:
            torch.save(net.state_dict(), "./weight/black_img/class4/model_c4_({})_({})_({}).pth".format(learning_rate, epoch, round(val_accuracy.item()*100,2)))
        elif val_accuracy == 1:
            break
    ...
    #가중치 저장
    torch.save(net.state_dict(), "model_c4_img_({})_({})_({}).pth".format(learning_rate, epochs, round(val_accuracy.item()*100,2)))
